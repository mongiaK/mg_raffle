...
static int sock = -1;
static int epfd = -1;
epoll_ctrl_callback _ep_callback = std::bind(
    &MEpollEvent::EpollCtl, &MEpollEvent::Instance(), std::placeholders::_1,
...
void do_response(MRequestSP req, M::RpcHeader &rpc_header,
    ...
    MBufferSP bufsp;
    bufsp = MBufferPool::Instance()->Allocate();
    if (bufsp == nullptr) {
    ...
    if (remain > 0) {
        req->GetConn()->SendQueue(
            std::make_shared<MBufferGuard>(bufsp, remain));
    } else {
        MBufferPool::Instance()->GiveBack(bufsp);
    }
}
...
void recv_run(int fd) {
    ...
    while (true) {
        ...
        for (int i = 0; i < retry; i++) {
            buf = MBufferPool::Instance()->Allocate();
            if (buf != nullptr)
                break;
        }
        if (buf == nullptr) {
        ...
        if (len < 0) {
            switch (errno) {
            case EAGAIN | EWOULDBLOCK:
                MBufferPool::Instance()->GiveBack(buf);
                return;
            case EINTR:
                MBufferPool::Instance()->GiveBack(buf);
                break;
