  ...
  public:
    ...
    int parser_msg(SBufferGuardSP bufsp, int &consume, int &remain_length) {
        ...
        case RPC_HEADER:
            ...
            real_read =
                std::min(g_header_size - _module_has_read_size, remain_length);
            ...
            if (_module_has_read_size < g_header_size) { // head read incomplete
                return 0;
            }
        ...
        case RPC_MESSAGE:
            real_read =
                std::min(_recieve_header._message_size - _module_has_read_size,
                         remain_length);
            _requestsp->get_read_buf()->Append(
                BufHandle(consume, real_read, bufsp));
